...
public class AimB extends AimCheck
{
    ...
    public AimB() {
        super(CheckType.AIM_ASSISTB, "B", "Aim Assist", CheckVersion.RELEASE);
        this.violations = -1.0;
    }
        ...
        if (playerData.getLastAttackTicks() < 3) {
            ...
            if (abs > 1.0f && (round = Math.round(abs)) == abs) {
                if (Objects.equals(abs, this.lastYawChange)) {
                    AlertsManager.getInstance().handleViolation(playerData, this, String.valueOf(new StringBuilder().append("ydiff:").append(decimalFormat.format(abs))), false);
                }
