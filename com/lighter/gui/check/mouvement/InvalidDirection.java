package com.lighter.gui.check.mouvement;
...
public class InvalidDirection extends Gui
{
    ...
    public void onClick(final ClickData clickData) {
        ...
        if (slot <= this.getInventory().getSize() && slot >= 0) {
            ...
            else if (clickData.getClickType().isLeftClick() && itemStack.getItemMeta().getDisplayName().equalsIgnoreCase(String.valueOf(new StringBuilder().append("§c§l").append(checkType.getName())))) {
            ...
            else if (clickData.getClickType().isRightClick() && itemStack.getItemMeta().getDisplayName().equalsIgnoreCase(String.valueOf(new StringBuilder().append("§c§l").append(checkType.getName())))) {
                clickData.getPlayer().closeInventory();
    ...
    public InvalidDirection() {
        super(String.valueOf(new StringBuilder().append(ChatColor.RED).append("InvalidDirection")), 18);
        ...
        for (final Check.CheckType checkType : Check.CheckType.values()) {
            if (checkType.getName().contains("InvalidDirection")) {
